...
class BaiduMapProvider extends MapBase {
    ...
    private uavList: Uav[];
    private getColor(value, opt: MapGridOptions, min: number = undefined, max: number = undefined) {
    ...
    private createBlock(point, opt) {
        ...
        var pcenter = new BMap.Point(lng, lat);
    ...
    gridClear() {
        if (this.blockGrid.blocks) {
            this.blockGrid.blocks.forEach(o => {
    ...
    uavExist(name: string): boolean {
        var obj = this.uavList.find((o) => o.name == name);
